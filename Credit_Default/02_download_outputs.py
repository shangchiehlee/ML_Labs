...
Author: Shane Lee
Description: Retrieves training artifacts from Azure Blob Storage.
             Executes a zero-byte integrity check on all downloaded files to validate transfer completeness.
             Generates a local manifest to establish the chain of custody.
"""
...
# Identify target job from local state file.
try:
    with open("logs/latest_job.txt", "r") as f:
        job_name = f.read().strip()
    logger.info(f"Target Job Identified: {job_name}")
except FileNotFoundError:
    logger.error("CRITICAL: State file 'logs/latest_job.txt' missing. Ensure '01_submit_job.py' has been executed.")
    sys.exit(1)
