...
Purpose:
    Utilises kernel-level memory mapping (mmap) to share model segments across worker
    processes, minimising the Resident Set Size (RSS). Aggregates predictions from the
    ensemble via soft voting and enforces strict contract validation.

Workflow:
    1. Loads the model artifact using `joblib` with mmap mode.
    2. Spawns a ProcessPool for parallel batch processing.
    3. Streams the input CSV in chunks.
    4. Workers calculate probabilities using the shared model.
    5. Main process aggregates results and writes to output CSV.
...
# --- Shared Worker State ---
# Global variables are required for ProcessPoolExecutor to share state via COW/mmap
...
def init_worker(model_path: Path) -> None:
    ...
    Uses zero-copy memory mapping to share the model artifact across parallel
    processes, significantly reducing physical RAM overhead.
    ...
    with pd.read_csv(input_file, index_col=0, chunksize=BATCH_SIZE) as reader:
        ...
        with ProcessPoolExecutor(
            ...
            for chunk in reader:
                ...
                # Backpressure Control: Prevent memory overload by limiting active futures
