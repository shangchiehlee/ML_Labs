...
Workflow:
    ...
    3. Writes processed chunks to `scoring_input_data.csv`.
    4. Writes failed chunks to `dlq_failed_chunks.csv`.
...
def execute_etl_stream(
    ...
    Args:
        ...
        output_path: Destination for inference data.
        dlq_path: Destination for failed chunk capture.
            ...
            with pd.read_csv(input_path, chunksize=batch_size, on_bad_lines='skip') as reader:
                for idx, chunk in enumerate(reader):
                    try:
                        ...
                        # Serialised write barrier to preserve ordering
                        if last_future:
                        ...
                        logger.error(f"DLQ EVENT: Batch {idx + 1} failed processing. Error: {ve}")
                        # Synchronous DLQ write to improve capture integrity
...
def main() -> None:
    ...
    default_input: Path = Path("inputs") / "feedback_stream_raw.csv"
    raw_input_value: str = feat_defs.get("input_path", str(default_input))
    raw_input: Path = Path(raw_input_value)
