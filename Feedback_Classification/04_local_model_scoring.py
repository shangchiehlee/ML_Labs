...
Purpose:
    Deserialises model artifacts using kernel-level memory mapping (mmap) to share
    memory segments across worker processes, minimising the Resident Set Size (RSS).
    Aggregates binary predictions and confidence scores with strict contract validation.

Workflow:
    1. Loads the model artifact (Pipeline) into shared memory.
