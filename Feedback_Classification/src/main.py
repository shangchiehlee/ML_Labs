"""Cloud Training Worker for Out-of-Core NLP Learning.

Purpose:
    Executes a memory-constant pipeline using stateless HashingVectorizer and
    PassiveAggressiveClassifier. Produces business-ready reporting artifacts
    directly from the data stream.

Architectural Constraints:
    - Memory Complexity: O(1) relative to dataset size.
    - Time Complexity: O(N) linear pass.
    - Vectorisation: Stateless (MurmurHash3) to avoid vocabulary storage.
...
def generate_drivers(
    ...
    """Extracts top bi-gram drivers for sentiment segments using a capped buffer.
    ...
    def extract_top_phrases(corpus: pd.Series, segment: str) -> None:
        if corpus.empty:
            return
        # bi-grams capture context like "not bad"
...
def process_stream(
    ...
    Args:
        ...
        audit_path: Path for disk-based result caching.
    ...
    # Capped Insight Buffers (Memory safety for BI extraction)
    ...
    for epoch in range(args.epochs):
        ...
        # Chunked Reader for O(1) Memory Complexity
        ...
        with pd.read_csv(args.data, chunksize=args.batch_size, on_bad_lines='skip') as reader:
            for chunk in reader:
                # Forensic Audit (Deduplication / Null Safety) established in Epoch 0
                if epoch == 0:
                ...
                # In-line transformation (Linear PASS)
...
def generate_artifacts(
    ...
    """Produces executive-ready reporting assets.
    ...
    """
    logger.info("Serialising executive artifacts...")

    # Priority Alerts: High confidence failures (False Positives/Negatives)
    try:
        priority_buffer: List[pd.DataFrame] = []
        if os.path.exists(audit_path):
            with pd.read_csv(audit_path, chunksize=50000) as reader:
                for chunk in reader:
                    # Logic: High confidence prediction (conf high), but wrong (true != pred)
                    # Or specific business rule: Negatives with high confidence
            ...
            if priority_buffer:
                ...
                priority_df.to_csv("outputs/priority_alerts.csv", index=False)
                mlflow.log_artifact("outputs/priority_alerts.csv")
    ...
    if not drivers_df.empty:
        drivers_df.to_csv("outputs/sentiment_drivers.csv", index=False)
        mlflow.log_artifact("outputs/sentiment_drivers.csv")
...
def main() -> None:
    ...
    # Stateless Vectoriser (MurmurHash3) - strict O(1)
    ...
    audit_path: str = "outputs/forensic_audit_full.csv"
    ...
    if y_true:
        ...
        logger.info(f"FINAL PERFORMANCE: ACC={acc:.4f}, F1={f1:.4f}, AUC={auc:.4f}")
        mlflow.log_metrics({"accuracy": acc, "f1_weighted": f1, "auc_roc": auc})
        ...
        # compress=0 is crucial for mmap support in 04_local_model_scoring.py
        joblib.dump(artifact, "outputs/model.joblib", compress=0)
        mlflow.log_artifact("outputs/model.joblib")
