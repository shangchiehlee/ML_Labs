...
Purpose:
    Streams raw datasets in chunks and writes normalised output for inference.
...
Workflow:
    ...
    3. Removes the "Serial No." column when present.
    4. Drops label column for blind inference simulation.
    5. Writes processed chunks to `scoring_input_data.csv`.
    6. Routes failed chunks to `dlq_failed_chunks.csv`.
            ...
            with pd.read_csv(data_url, chunksize=batch_size) as reader:
                for idx, chunk in enumerate(reader):
                    try:
                        ...
                        # Serialised write barrier (strict ordering)
                        if last_future:
