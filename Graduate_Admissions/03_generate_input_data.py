...
Workflow:
    ...
    2. Normalises header names (strips quotes, whitespace).
    3. Removes non-predictive columns (Serial No.).
    4. Drops label column for blind inference simulation.
    5. Writes valid chunks to `scoring_input_data.csv`.
    6. Isolates malformed chunks to `dlq_failed_chunks.csv`.
...
DISK_TIMEOUT: int = ops_settings.get("disk_serialisation_timeout_seconds", 60)
...
def execute_etl_stream(
    ...
    with ThreadPoolExecutor(max_workers=1) as executor:
        ...
        try:
            ...
            with pd.read_csv(data_url, chunksize=batch_size) as reader:
...
def main() -> None:
    ...
    target_col: str = feature_defs.get("target", "Chance of Admit")
