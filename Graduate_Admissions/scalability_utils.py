...
Purpose:
    Implements the utility library used by module scripts.
    This module is intentionally duplicated in module-level directories and
    copied into the module `src` directory during job submission so it is included in the Azure ML
    packaging boundary. This avoids runtime dependency on the repository-level
    `Utils` directory.
...
try:
    ...
    # Azure SDK imports are optional and only required by local orchestration scripts.
