"""Cloud Training Worker for Out-of-Core Graduate Admissions Prediction.

Purpose:
    Executes incremental learning via SGDRegressor with adaptive learning rates.
    Calculates dual-task metrics (Regression and Classification) from the data
    stream to ensure high-precision convergence and O(1) space complexity.

Architecture:
    - Incremental Learning: partial_fit() on streaming batches.
    - Memory Complexity: O(1) via chunked generation.
    - Evaluation: Simultaneous regression (R2, MSE) and classification (Accuracy)
      metrics derivation based on a success threshold.
...
def main() -> None:
    ...
    # compress=0 enables mmap local scoring in 04_local_model_scoring.py
