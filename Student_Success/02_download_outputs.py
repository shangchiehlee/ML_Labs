"""Retrieval and audit utility for cloud artefacts.

Purpose:
    Retrieves job artefacts from Azure ML jobs.
    Flags zero-byte files and generates a local manifest with file metadata.

Workflow:
    1. Identifies the latest job ID using utils.get_latest_job_id.
...
def perform_integrity_check(local_path: Path, job_name: str) -> Tuple[Manifest, bool]:
    ...
    Returns:
        Tuple[Manifest, bool]: Audit manifest and integrity status.
    """
    ...
    file_paths: List[Path] = sorted(
        ...
        key=lambda path: str(path.relative_to(local_path))
    )
...
def main() -> None:
    ...
    # Forensic Audit
    manifest_data, integrity_ok = perform_integrity_check(download_dir, job_name)
