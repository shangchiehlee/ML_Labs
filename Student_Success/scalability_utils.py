...
try:
    ...
    # Azure SDK imports are optional to allow execution without the SDK installed.
...
def configure_logging(
    ...
    Args:
        ...
        log_filename: Filename for the log file in the resolved log directory.
...
def get_memory_limit() -> int:
    ...
    try:
        cgroup1: Path = Path(
            os.sep, "sys", "fs", "cgroup", "memory", "memory.limit_in_bytes"
        )
        if cgroup1.exists():
            limit: int = int(cgroup1.read_text().strip())
            # Filter out "unlimited" values
            if limit < 1e15:
