...
Module Purpose:
    Implements the **Self-Contained Utility Pattern** for Azure ML execution.
    This module is intentionally duplicated in module-level directories to be
    packaged with the training script (`src/`) during deployment. This ensures
    atomic, reproducible deployments without complex path dependencies on the
    root `Utils/` directory, which may not be accessible in the isolated
...
def get_memory_limit() -> int:
    ...
    try:
        ...
        if cgroup1.exists():
            ...
            # Filter out "unlimited" values (often > 1PB in containers)
            if limit < 1e15:
