"""Cloud Training Worker for Out-of-Core Student Outcome Prediction.

Purpose:
    Executes multi-class classification using SGDClassifier with incremental learning
    (partial_fit). Implements a memory-constant evaluation strategy utilising a
    Cumulative Confusion Matrix to ensure scalability regardless of input volume.

Architecture:
    - Incremental Learning: partial_fit() on streaming batches.
    - Memory Complexity: O(1) via chunked generation.
    - Evaluation: Aggregated confusion matrix for precision/recall derivation.
...
def discover_classes(
    ...
    try:
        # Utilise chunked reader to maintain O(1) memory footprint during discovery
...
def main() -> None:
    ...
    # Aggregated state for O(1) Memory Metric calculation
    ...
    acc, prec, rec, f1 = calculate_weighted_metrics(
        cumulative_conf_matrix, support, int(np.sum(cumulative_conf_matrix))
    )

    logger.info(f"SCORING COMPLETE - Accuracy: {acc:.4f} | F1: {f1:.4f}")
