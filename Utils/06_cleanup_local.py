...
Purpose:
    Executes a hard reset of the local environment by recursively removing
    transient data directories, logs, and build artifacts.

Safety Mechanisms:
    *   Operates without external dependencies to avoid file-locking deadlocks.
    *   Implements recursive permission modification to handle read-only Azure artifacts.
    *   Avoids file handlers for logging to prevent self-lock.
...
def remove_readonly(func: Callable, path: str, excinfo: Any) -> None:
    ...
    If a file is Read-Only (common with retrieved Azure artifacts), this forces
    write permissions and retries the deletion.
...
def main() -> None:
    ...
    # 2. Build Artifacts
    # Note: root Utils doesn't typically have a src/ copy of scalability_utils,
    # but we check for it to ensure monorepo-wide script consistency.
