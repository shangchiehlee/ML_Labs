"""Connectivity Diagnostic for Azure Machine Learning.

Purpose:
    Validates the resolution chain for DefaultAzureCredential and performs a
    latency-indexed handshake with the target Workspace. Ensures that the
    local environment possesses the necessary IAM permissions to orchestrate
    Cloud workloads.

Workflow:
    1. Loads local configuration.
    2. Validates essential keys (subscription, resource group, workspace).
    3. Resolves Azure credentials using the default chain (CLI -> Env -> Managed Identity).
...
def main() -> None:
    ...
    try:
        ...
        # Note: get_azure_client returns an object even if auth fails later.
        # The first real API call will trigger the authentication check.
