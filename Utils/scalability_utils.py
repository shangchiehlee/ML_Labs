"""Master Library for Scalability and Resource Management.

Module Purpose:
    Provides foundational utilities for the ML_Labs monorepo, including
    container-aware resource detection, probabilistic data structures, and
    structured logging. This module serves as the authoritative source for the
    **Self-Contained Utility Pattern**.

    *Note: This file is the 'Golden Copy'. Orchestrators in other modules
    inject copies of this file into their `src/` directories during build.*

Key Capabilities:
    *   **Container Governance:** Cgroup V1/V2 memory limit detection.
    *   **Probabilistic Structures:** Bloom Filters for O(m) stream deduplication.
    *   **Observability:** Structured JSON/Text logging factories.
...
try:
    ...
    # These are mandatory for root-level diagnostics and orchestration
...
class ProjectPaths:
    ...
    Paths are resolved relative to the location of this script to ensure
    stability across different execution contexts (local vs remote).
    """
    ...
    # Audit & State
    JOB_HISTORY: Path = LOGS / "job_history.csv"
...
def configure_logging(
    ...
    Args:
        ...
        log_filename: Filename for the log file (stored in logs/).
    ...
    """
    # 1. Resolve Log Directory (Config > ProjectPaths)
    ...
    logger.handlers = []  # idempotent: clear existing handlers
...
def load_config(path: PathLike = "config.json") -> ConfigType:
    ...
    if not config_path.exists():
        # Fallback to template check for user guidance
        ...
        print(f"CRITICAL: Configuration file '{config_path}' missing.", file=sys.stderr)
        sys.exit(1)
    ...
    try:
        ...
        print(f"CRITICAL: Configuration file '{config_path}' is not valid JSON: {e}", file=sys.stderr)
        sys.exit(1)
    except Exception as e:
...
def _validate_config_version(
    ...
    if version != expected_version:
        # We log a warning for version mismatch to maintain forward compatibility.
...
def get_azure_client(config: ConfigType) -> Any:
    """Authenticated factory to create MLClient.
    ...
    try:
        ...
        print(f"CRITICAL: Azure Client Instantiation Failed: {e}", file=sys.stderr)
        sys.exit(1)
...
def validate_dataframe_schema(
    ...
    """Validates that the dataframe conforms to the contract defined in configuration.
...
def check_disk_space(path: PathLike = ".", min_gb: float = 1.0) -> None:
    """Ensures sufficient disk space exists.
...
def get_memory_limit() -> int:
    ...
    # Try Cgroup V2
    try:
        cgroup2: Path = Path("/sys/fs/cgroup/memory.max")
        if cgroup2.exists():
    ...
    # Try Cgroup V1
    try:
        cgroup1: Path = Path("/sys/fs/cgroup/memory/memory.limit_in_bytes")
        if cgroup1.exists():
            limit: int = int(cgroup1.read_text().strip())
            # Filter out "unlimited" values (often > 1PB)
            if limit < 1e15:
...
def calculate_optimal_batch_size(
    ...
    # Clamp to reasonable limits to prevent overhead or timeouts
...
def resolve_batch_size(
    ...
    """Determines batch size prioritising configuration over dynamic calculation.
    ...
    config_batch: Any = ops_settings.get("batch_size")
    ...
    return calculate_optimal_batch_size(
        n_jobs, 0.5, row_size_estimate_bytes
    )


def find_model_artifact(base_path: PathLike) -> Optional[Path]:
    """Recursively searches for the model artifact in a directory tree.
    ...
    # rglob returns a generator, next() gets the first item
