...
Mechanisms:
    ...
    *   **Resilience:** Decorator for exponential backoff.
...
try:
    ...
    # Optional dependencies used by local diagnostics and orchestration scripts.
...
class ProjectPaths:
    ...
    stability across different execution contexts.
    """
...
def load_config(path: PathLike = "config.json") -> ConfigType:
    """Loads the configuration file and checks the schema version.
...
def _validate_config_version(
    ...
    if version != expected_version:
        # Log a warning when the schema version does not match expectations.
...
def get_azure_client(config: ConfigType) -> Any:
    """Factory to create MLClient using DefaultAzureCredential.

    Args:
        config: Configuration dictionary containing Azure credentials.

    Returns:
        MLClient: Azure ML client configured with DefaultAzureCredential.
    """
...
def validate_dataframe_schema(
    ...
    """Validates required columns and attempts dtype casting from configuration.
...
def get_memory_limit() -> int:
    ...
    Falls back to host RAM when cgroup limits are unavailable.
    ...
    try:
        ...
        if cgroup1.exists():
            ...
            # Filter out large "unlimited" values
            if limit < 1e15:
...
def calculate_optimal_batch_size(
    ...
    # Clamp to reasonable limits to reduce overhead or timeouts
...
def find_model_artifact(base_path: PathLike) -> Optional[Path]:
    """Recursively searches for the model artefact in a directory tree.
...
class BloomFilter:
    """A probabilistic data structure for membership testing (O(m) space)."""
...
class StreamForensics:
    ...
    def audit_chunk(self, df: pd.DataFrame) -> pd.DataFrame:
        """Audits a data chunk, removing nulls and repeated indices.
